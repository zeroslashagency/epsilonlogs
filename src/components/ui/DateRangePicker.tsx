...
export function DateRangePicker({
  ...
  const handleStartChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    onChangeStruct(fromNative(e.target.value), endDate);
  };

  const handleEndChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    onChangeStruct(startDate, fromNative(e.target.value));
  };

  return (
    <div
      className={`flex flex-col sm:flex-row gap-4 items-end sm:items-center p-2 rounded-lg border ${containerClass}`}
    >
      {/* Start Date */}
      <div className="w-full sm:w-auto">
        <label
          className={`block text-xs font-semibold mb-1 flex items-center gap-1 ${labelClass}`}
        >
        ...
        <div className="relative">
          ...
          />
          <Clock
            className={`absolute left-2.5 top-2.5 h-4 w-4 pointer-events-none ${iconClass}`}
          />
        </div>
      </div>
