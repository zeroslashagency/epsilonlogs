...
export function ReportTable({ rows, loading, isFiltered }: ReportTableProps) {
  if (rows.length === 0) {
    if (isFiltered) {
      return (
        <div className="w-full h-20 flex items-center justify-center text-slate-400 border rounded-lg bg-slate-50 text-sm">
            ...
            {rows.map((row, idx) => {
                  ...
                  <tr key={row.rowId} className="bg-blue-600 text-white">
                    <td colSpan={10} className="px-4 py-3">
                      <div className="flex items-center gap-4 flex-wrap font-semibold text-sm">
                        ...
                        <span>
                          Part: <strong>{h.partNo}</strong>
                        </span>
                        <span>
                          Operator: <strong>{h.operatorName}</strong>
                        </span>
                        <span>
                          PCL: <strong>{h.pclText}</strong>
                        </span>
                        <span>
                          Setting: <strong>{h.setting}</strong>
                        </span>
                        <span className="text-blue-200 text-xs">
                          Device {h.deviceId}
                        </span>
                        {h.startComment && (
                          <span className="bg-blue-700 px-2 py-0.5 rounded text-xs italic">
                            📝 {h.startComment}
                          </span>
                        )}
                      </div>
              ...
              if (row.isWoSummary && row.woSummaryData) {
                ...
                return (
                  <tr key={row.rowId} className="bg-slate-800 text-white">
                    <td
                      colSpan={10}
                      className="px-3 py-2 bg-slate-800/95 border-y border-slate-700"
                    >
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex flex-col items-start min-w-[140px]">
                          ...
                          {s.startComment && (
                            <div
                              className="text-[10px] italic text-slate-400 mt-0.5 max-w-[150px] truncate"
                              title={s.startComment}
                            >
                        ...
                        <div className="col-span-6 flex flex-col items-center justify-center text-center">
                          ...
                          <div className="flex items-center gap-3 text-sm text-slate-400 bg-slate-900/50 px-3 py-1 rounded-full border border-slate-700/50 mt-1">
                            <span>
                              Jobs:{" "}
                              <b className="text-slate-200">{s.totalJobs}</b>
                            </span>
                            <span className="text-slate-600">/</span>
                            <span>
                              Cyc:{" "}
                              <b className="text-slate-200">{s.totalCycles}</b>
                            </span>
                            {(s.keyEventsTotal || 0) > 0 && (
                              <>
                                ...
                                <span className="text-slate-600">
                                  ({s.keyOnCount || 0}/{s.keyOffCount || 0})
                                </span>
                              </>
                            )}
                            <span className="text-slate-600 mx-1">|</span>
                            <span>
                              Allot:{" "}
                              <b className="text-slate-200">{s.allotedQty}</b>
                            </span>
                            <span className="text-slate-600">→</span>
                            <span
                              className={
                    ...
                    <td colSpan={10} className="px-4 py-2">
                      ...
                      >
                        ...
                        <div className="flex flex-col">
                          <span>
                            {p.isShiftBreak ? "SHIFT BREAK" : "WO_PAUSE"}
                          </span>
                          <div className="flex flex-col mt-0.5 ml-0.5">
                            <div className="flex gap-1 text-xs font-normal text-slate-700">
                              ...
                              <span className="font-medium underline decoration-slate-400/50 underline-offset-2">
                                {p.reason}
                              </span>
                            </div>
              ...
              return (
                <tr
                  ...
                  style={
                    isInBlock
                      ? { backgroundColor: "rgb(199, 255, 216)" }
                      : undefined
                  }
                >
                  {/* S.No */}
                  ...
                  <td className="px-3 py-2 font-mono text-slate-700 align-top min-w-[100px]">
                    {row.startRowData ? (
                      <div className="flex flex-col space-y-0.5">
                        <span className="text-slate-500 font-medium uppercase text-[9px] tracking-wider">
                          Part No
                        </span>
                        <span className="font-bold text-slate-800 text-[11px] leading-tight">
                          {row.startRowData.partNo}
                        </span>
                      </div>
                    ) : row.stopRowData ? (
                      <div className="flex flex-col space-y-0.5">
                        <span className="text-emerald-600 font-medium uppercase text-[9px] tracking-wider">
                          OK Qty
                        </span>
                        <span className="font-bold text-slate-800 text-[11px] leading-tight">
                          {formatHumanEntry(row.stopRowData.ok)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  ...
                  <td className="px-3 py-2 text-center text-slate-700 align-top">
                    {isFirstInBlock && row.jobBlockLabel ? (
                      ...
                      <div className="flex flex-col space-y-0.5 text-xs text-left">
                        ...
                        <span className="font-bold text-slate-800">
                          {formatHumanEntry(row.stopRowData.reject)}
                        </span>
                      </div>
                    ) : (
                      ...
                      <div className="flex flex-col space-y-1">
                        <div className="flex flex-col space-y-0.5">
                          ...
                          <span className="italic text-slate-700">
                            {formatHumanEntry(row.startRowData.comment)}
                          </span>
                        </div>
                        {row.durationText && (
                          <span className="font-mono font-bold text-slate-900 border-t border-slate-200 mt-1 pt-0.5 w-fit">
                            ⏲ {row.durationText}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col space-y-1">
                        <div className="flex flex-col space-y-0.5">
                          ...
                          <span className="font-medium text-slate-800">
                            {formatHumanEntry(row.stopRowData.reason)}
                          </span>
                        </div>
                        {row.durationText && (
                          <span className="font-mono font-bold text-slate-900 border-t border-slate-200 mt-1 pt-0.5 w-fit">
